...
class VKGetAge(BaseAnalysisTask):
    ...
    def _create_search_params_from_userinfo(self, info, age_from=0, age_to=70):
    ...
    def _get_age_by_info(self, info):
        ...
        log.debug(f"Определен возраст пользователя по поиску (uid={uid}). Приблизительный возраст={age}")
        if age is None:
            return None
        elif age == 0:
            return None
    ...
    def _simple_get_age_of_vk_female_users(self, user_ids: list):
        """"
            Simple method to determine age of users
            return: age or error or None
        """
        result = []
        all_info = self.vk_module.get_users(user_ids, ["bdate", "sex"])["result"]
    ...
    def simple_get_age_by_user(self, all_info):
        """"
            Simple method to determine age of users
            return: age or error or None
        """
        #указана дата в профиле
        if all_info.get("bdate"):
            bday = tuple(map(int, all_info["bdate"].split('.')[::-1]))
            if len(bday) == 3:
                bday = datetime.datetime(*bday)
                age = floor(((self.TODAY_DATE.year - bday.year) * 12 + (self.TODAY_DATE.month - bday.month)) / 12)
                if self.LEFT_SIDE_OF_AGE <= age and age <= self.RIGHT_SIDE_OF_AGE:
    ...
    def get_age_of_vk_user(self, user_info, num_of_iter: int=2):
        """"
            Determine age of user
            return: age
        """

        #пытаемся определить по поиску - выдаёт 0 почему-то
        # try:
        #     age_simple = self._get_age_by_info(user_info)
        #     if age_simple is not None:
        #         return age_simple
        # except Exception as e:
        #     log.error(f"Ошибка при определении возраста пользователя {user_info['id']} по поиску. Ошибка: {e}")

        #не удалось определить простым способом
        try:
            year_grad = self._det_year_graduated([user_info.get('id')])
            if year_grad:
            #указан год окончания школы
                year_by_school = round(year_grad[0] * self.a + self.b_1)
                age = self.TODAY_DATE.year - year_by_school
                log.debug(f"Определен примерный возраст пользователя по учёбе (uid={user_info.get('id')}). Возраст={age}")
                return age
        except Exception as e:
            log.error(f"Ошибка при определении возраста пользователя {user_info['id']} по учёбе. Ошибка: {e}")

        #ничего не указано, итеративный способ
        try:
            age = None
            friends_of_target = self.vk_module.get_friends(user_info.get('id'))["result"]
            log.debug(f"Функция get_friends вернула: {friends_of_target}")
            if friends_of_target:
                friends_of_target_ages_dict = self._simple_get_age_of_vk_female_users(friends_of_target)
                ...
                if len(friends_of_target_ages) >= 3:
                    ...
                    if friends_of_target_ages:
                        ...
                        age = fr_age
                        log.debug(f"Определен примерный возраст пользователя по друзьям (uid={user_info.get('id')}). Возраст={age}")
                        if num_of_iter == 1:
                            return age

                    if num_of_iter >= 2:
                        new_friends_ages_dict = self._simple_get_age_of_vk_female_users(friends_without_age)
                        ...
                        if friends_of_target_ages and new_friends_ages:
                            ...
                            if age:
                                ...
                                age = fr_age
                            log.debug(f"Определен примерный возраст пользователя по друзьям 2-ой итерацией (uid={user_info.get('id')}). Возраст={age}")

                return age
        except Exception as e:
            log.error(f"Ошибка при определении возраста пользователя {user_info['id']} по друзьям. Ошибка: {e}")
        
    def execute(self, user_info, db_user):
        try:
            if type(user_info) is not dict:
                raise Exception("Error getting user info for age calculation")            

            if user_info.get("deactivated"):
                log.debug(f"Профиль пользователя заблокирован!")
                return
            
            age_result_simple = self.simple_get_age_by_user(user_info)
            if age_result_simple:
                return age_result_simple
            
            if user_info.get("is_closed") and not user_info.get("can_access_closed"):
                log.debug(f"Профиль пользователя скрыт!")
                return            
            
            if not age_result_simple and not db_user:
                if user_info.get('sex') == 1:
                    age = self.get_age_of_vk_user(user_info)
                    if age:
                        return round(age, 3)            
        
        except Exception as e:
            log.error(f"Ошибка при определении возраста пользователя {user_info['id']}. Ошибка: {e}")
            return None