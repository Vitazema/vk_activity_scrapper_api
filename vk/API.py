...
class VK:
        ...
        while attempt < 3:
            ...
            if "req_err" in data:
                ...
                if data["error"]["error_code"] == 30:
                    ...
                    log.warning(f"Too many executes, waiting... attempt: {attempt + 1}")
                    time.sleep(1)
                    attempt += 1
                    continue
    ...
    def resolve_screen_name(self, screen_name: str):
        ...
        Arguments:
            screen_name: str – screen name of user or group
        Return:
            result: dict – {result: id, error: {id: text_error, ...}, status: success or fail}
        """
        req_data = {
            "screen_name": screen_name
        }
        try:
            data = self._vkapi_request("utils.resolveScreenName", req_data)
            return self.result("success", data["response"]["object_id"], None)
        except VkApiProfileIsPrivate:
            return self.result("fail", None, {screen_name: "Profile is private"})
        except VkApiDeletedUser:
            return self.result("fail", None, {screen_name: "User is deleted"})
        except VkApiBannedUser:
            return self.result("fail", None, {screen_name: "User is banned"})
        except VkApiInaccessibleContent:
            return self.result("fail", None, {screen_name: "Inaccessible content"})
        except VkApiNoAdmission:
            return self.result("fail", None, {screen_name: "No admission"})
        except VkInvalidToken:
            return self.result("fail", None, {screen_name: "Invalid token"})
        except BaseVkError as e:
            return self.result("fail", None, {screen_name: f"Unknown error: {e}" },)

    def get_cities_by_id(self, city_ids: list):
        req_data = {
            "city_ids": city_ids
        }
        try:
            data = self._vkapi_request("database.getCitiesById", req_data)
            return self.result("success", data["response"], None)
        except VkApiProfileIsPrivate:
            return self.result("fail", None, {city_ids: "Profile is private"})
        except VkApiDeletedUser:
            return self.result("fail", None, {city_ids: "User is deleted"})
        except VkApiBannedUser:
            return self.result("fail", None, {city_ids: "User is banned"})
        except VkApiInaccessibleContent:
            return self.result("fail", None, {city_ids: "Inaccessible content"})
        except VkApiNoAdmission:
            return self.result("fail", None, {city_ids: "No admission"})
        except VkInvalidToken:
            return self.result("fail", None, {city_ids: "Invalid token"})
        except BaseVkError:
            return self.result("fail", None, {city_ids: "Unknown error"})
    ...
    def get_group_posts(self, domain_name: str, max_posts: int=120):
        """
        Collect posts of group
        Arguments:
            group_id: int – id of group
            max_posts: int - num of returned posts
        Return:
            result: dict – {result: [posts_id], error: {id: text_error, ...}, status: success or fail}
        """
        posts = []
        if max_posts < 100:
            posts_per_iteration = max_posts
        else:
            posts_per_iteration = 100

        res = []
        cur_offset = 0

        #     buff = self._vkapi_request("wall.get", {"owner_id": -group_id, "count": 100, "offset": cur_offset})
        #     if "items" in buff:
        #         res += buff["items"]
        #     cur_offset += 100
        # return self.result("success", res[:max_posts], None)
    
        # try:
        #     posts = self._vkapi_request("execute", {"code": code})


        try:
            while len(res) < max_posts and cur_offset < 2500:
                req_data = {
                    "domain": domain_name,
                    "count": posts_per_iteration,
                    "offset": cur_offset
                }
                buff = self._vkapi_request("wall.get", req_data)
                if "items" in buff:
                    res += buff["items"]
                    cur_offset += 100
            ...
            return self.result("fail", None, ["Unknown error"])
        if "req_err" in posts:
            return self.result("fail", None, [posts])
        log.debug(f'group_id: {domain_name}; max_posts: {max_posts} -> OK')
        return self.result("success", posts, None)
        
    def get_groupById(self, group_id):
        req_data = {}
        ...
        try:
            data = self._vkapi_request("groups.getById", req_data)
            response = data['response']
            if response == []:
                return self.result("fail", None, [{group_id: "100: Invalid group id"}])
            return self.result("success", data["response"], None)
        except VkApiProfileIsPrivate:
